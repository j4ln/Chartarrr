...
function displayComment(comment) {
  ...
  document
    ...
    .replaceWith(createCommentElement(comment, comment_index));
}
...
function displayEndSession() {
  ...
  getActiveSession().then((session) => {
    ...
    document
      .getElementById("button-preview-md")
      .addEventListener("click", () => {
        session = addCharterDetailsToSession(session);
        document.getElementById("preview").style.display = "block";
        document.getElementById("preview-content").textContent =
          getMarkdownExport(session);
      });
    document
      .getElementById("button-preview-json")
      .addEventListener("click", () => {
        session = addCharterDetailsToSession(session);
        document.getElementById("preview").style.display = "block";
        document.getElementById("preview-content").textContent = JSON.stringify(
          session,
          null,
          4,
        );
...
function loadDetails(session) {
  ...
  document.getElementById("start-date").value =
    session.datetime_started.substring(0, 16);
  document.getElementById("end-date").value = new Date()
    .toISOString()
    .substring(0, 16);
}
...
function downloadExport(session) {
  ...
  session.comments.forEach((comment, index) => {
    if (comment.attachment) {
      const imageBase64 = comment.attachment.replace(
...
function addCharterDetailsToSession(session) {
  ...
  session.estimated_duration = Number(
    document.getElementById("estimated-duration").value,
  );
